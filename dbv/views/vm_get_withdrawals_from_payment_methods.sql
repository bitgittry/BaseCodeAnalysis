DROP view IF EXISTS `vm_get_withdrawals_from_payment_methods`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`localhost` VIEW `vm_get_withdrawals_from_payment_methods` AS
 SELECT 
        ...
        IFNULL(`gaming_payment_transaction_status`.`name`, 'Pending') AS `TransactionStatus`,
        `gaming_payment_method`.`name` AS `PaymentMethod`,
        `gaming_balance_manual_transactions`.`reason` AS `StatusReason`,
         ROUND ( `gaming_balance_history`.`balance_real_after` , 2) AS `PlayerBalance`,
        ...
        `gaming_balance_manual_transactions`.`transaction_date` AS `transactiondate`,
        `gaming_operator_currency`.`exchange_rate` AS  `exchangerateOperator`,
    ...
    WHERE
        `gaming_balance_manual_transactions`.`payment_transaction_type_id` IN (2 , 18) 
