DROP procedure IF EXISTS `CommonWalletCommonPlaceBet`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletCommonPlaceBet`(
  gameSessionKey VARCHAR(40), clientStatID BIGINT, gameManufacturerName VARCHAR(80), transactionRef VARCHAR(80), roundRef BIGINT, 
  gameRef VARCHAR(80), betAmount DECIMAL(18, 5), jackpotContribution DECIMAL(18, 5), ignoreSessionExpiry TINYINT(1), 
  canCommit TINYINT(1), minimalData TINYINT(1), OUT statusCode INT)
root: BEGIN
 
  DECLARE operatorGameID, sessionID, gameID, gameManufacturerID, clientStatIDCheck, currencyID, oldRoundRef BIGINT DEFAULT -1;
