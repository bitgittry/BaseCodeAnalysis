DROP procedure IF EXISTS `TournamentGetAllTournamentForNotLoggedInPlayers`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `TournamentGetAllTournamentForNotLoggedInPlayers`(currentOnly TINYINT(1))
BEGIN
    ...
    SELECT gaming_tournaments.tournament_id, gaming_tournaments.name, gaming_tournaments.display_name, gaming_tournaments.tournament_type_id,
           ...
           prize_type, gaming_tournaments.automatic_award_prize, gaming_tournaments.wager_req_real_only,
           gaming_tournaments.currency_profile_id, gaming_tournaments.game_weight_profile_id, sb_weight_profile_id, tournament_types.name AS tournament_type_name
