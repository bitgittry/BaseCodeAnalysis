DROP function IF EXISTS `TransactionCheckDepositAmountWithinLimit`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` FUNCTION `TransactionCheckDepositAmountWithinLimit`(
  clientStatID BIGINT, paymentMethodID BIGINT, varAmount DECIMAL(18, 5), balanceAccountID BIGINT) RETURNS int(11)
    READS SQL DATA
    DETERMINISTIC
BEGIN
  ...
  SET @dateYearFilter=DATE_FORMAT(NOW() ,'%Y-01-01');
  ...
  IF (transferLimitsEnabled=1) THEN
        ...
        -- to be able to reduce the number of transactions retrieved for computing the limit remaining
        SET @transactionFilter=NULL;
        SELECT txn_filter INTO @transactionFilter
		FROM 
		(
		  SELECT 
			CASE 
		  ...
		  STRAIGHT_JOIN gaming_interval_type ON gaming_interval_type.interval_type_id=gaming_transfer_limit_clients.interval_type_id
		  WHERE gaming_transfer_limit_clients.client_stat_id=@clientStatID
				  AND gaming_transfer_limit_clients.is_active=1 
				  AND gaming_transfer_limit_clients.is_confirmed = 1
				  AND((gaming_transfer_limit_clients.end_date IS NOT NULL AND gaming_transfer_limit_clients.end_date >= NOW()) OR 
					(gaming_transfer_limit_clients.end_date IS NULL AND gaming_transfer_limit_clients.start_date <= NOW()))
		) AS XX
		ORDER BY XX.txn_filter ASC 
		LIMIT 1;
        
        SET @paymentMethodParentID=NULL;
        SELECT parent_payment_method_id INTO @paymentMethodParentID FROM gaming_payment_method WHERE payment_method_id=@paymentMethodID AND is_sub_method=1;
		SET @paymentMethodParentID=IFNULL(@paymentMethodParentID, @paymentMethodID);
      
	  SELECT
		PlayerStatus.payment_method_id,
		PlayerStatus.admin_min_limit AS min_amount,
		MIN(PlayerStatus.remaining) AS max_amount,
		gaming_payment_method.can_deposit  
	  INTO
	 	@paymentMethodIDCheck, @minAmount, @maxAmount, @canDeposit
	  FROM
	  (
		SELECT 
		...
		IF(gaming_transfer_limit_clients.limit_amount IS NULL,
			LEAST(gaming_payment_amounts.max_deposit, IFNULL(gaming_transfer_limit_amounts.admin_max_amount,gaming_payment_amounts.max_deposit)) - 
				IFNULL(IF(TotalValue.transfer_limit_client_id IS NULL, CurrentValue.deposited_amount, TotalValue.deposited_amount), 0),
			LEAST(gaming_payment_amounts.max_deposit, IFNULL(gaming_transfer_limit_amounts.admin_max_amount,gaming_payment_amounts.max_deposit),gaming_transfer_limit_clients.limit_amount) - 
				IFNULL(IF(TotalValue.transfer_limit_client_id IS NULL, CurrentValue.deposited_amount, TotalValue.deposited_amount), 0)
		  ) AS remaining
		FROM gaming_client_stats 
		...
		STRAIGHT_JOIN gaming_interval_type ON 
			gaming_interval_type.is_transfer_limit=1
		STRAIGHT_JOIN gaming_payment_transaction_type ON 
			gaming_payment_transaction_type.name='Deposit'
		STRAIGHT_JOIN gaming_payment_amounts ON 
			gaming_payment_amounts.currency_id=gaming_client_stats.currency_id 
		STRAIGHT_JOIN gaming_payment_method ON 
			gaming_payment_method.payment_method_id IN (@paymentMethodID, @paymentMethodParentID) AND
			gaming_payment_method.is_payment_gateway_method=1 AND gaming_payment_method.is_active=1 
		...
		(
		  ...
		  (
			...
			STRAIGHT_JOIN gaming_balance_history FORCE INDEX (player_transaction_type_date) ON 
              ...
              (gaming_balance_history.client_stat_id=@clientStatID AND
               ...
               gaming_balance_history.timestamp>@dateYearFilter) 
				 AND (gaming_balance_history.payment_method_id=@paymentMethodParentID OR gaming_balance_history.sub_payment_method_id=@paymentMethodID)
			STRAIGHT_JOIN gaming_payment_transaction_status ON 
			...
			DL.payment_method_id=BalanceTransactions.payment_method_id AND 
            BalanceTransactions.timestamp>=DL.transaction_filter_start_date AND 
            (DL.create_date IS NULL OR BalanceTransactions.timestamp>DL.create_date)
		...
		(
		  ...
		  (
			...
			STRAIGHT_JOIN gaming_clients ON
			  gaming_client_stats.client_stat_id=@clientStatID AND
			  gaming_client_stats.client_id=gaming_clients.client_id
			STRAIGHT_JOIN gaming_interval_type ON gaming_interval_type.is_transfer_limit=1
			STRAIGHT_JOIN gaming_transfer_limit_clients 
			  ON  gaming_transfer_limit_clients.interval_type_id=gaming_interval_type.interval_type_id 
			  AND gaming_transfer_limit_clients.client_stat_id=gaming_client_stats.client_stat_id 
			  AND gaming_transfer_limit_clients.is_active=1 
			  AND gaming_transfer_limit_clients.is_confirmed = 1
		  ...
		  LEFT JOIN 
		  (
			SELECT gaming_balance_history.balance_history_id, amount, timestamp, gaming_balance_history.payment_method_id  
			FROM gaming_payment_transaction_type
			STRAIGHT_JOIN gaming_balance_history FORCE INDEX (player_transaction_type_date) ON 
			  gaming_payment_transaction_type.name='Deposit' AND
              (gaming_balance_history.client_stat_id=@clientStatID AND
			   gaming_balance_history.payment_transaction_type_id=gaming_payment_transaction_type.payment_transaction_type_id AND
               gaming_balance_history.timestamp>@transactionFilter)
			STRAIGHT_JOIN gaming_payment_transaction_status ON 
			  gaming_payment_transaction_status.name IN ('Accepted','Authorized_Pending') AND
			  gaming_balance_history.payment_transaction_status_id=gaming_payment_transaction_status.payment_transaction_status_id 
  ...
  ELSE
	  ...
	  SELECT gaming_payment_method.payment_method_id, gaming_payment_amounts.min_deposit, gaming_payment_amounts.max_deposit, gaming_payment_method.can_deposit
	  INTO @paymentMethodIDCheck, @minAmount, @maxAmount, @canDeposit
	  FROM gaming_client_stats
