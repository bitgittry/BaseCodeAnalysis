DROP procedure IF EXISTS `PlayReturnDataWithoutGameForSbExtraID`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlayReturnDataWithoutGameForSbExtraID`(
  sbExtraID BIGINT, licenseTypeID TINYINT(4), gameRoundID BIGINT, clientStatID BIGINT, gameManufacturerID BIGINT, minimalData TINYINT(1))
BEGIN
  ...
  ELSE 
	  ...
	  SELECT game_play_id, game_round_id, gaming_payment_transaction_type.name AS transaction_type, amount_total, amount_total_base, 
		...
		bonus_lost, bonus_win_locked_lost, timestamp, exchange_rate, game_id, game_manufacturer_id, operator_game_id, 
		client_stat_id, game_session_id, balance_real_after, balance_bonus_after, is_win_placed, is_processed, game_play_id_win, 
		amount_tax_operator, amount_tax_player, loyalty_points, loyalty_points_after, loyalty_points_bonus, loyalty_points_after_bonus, pending_winning_real,
		sb_bet_id, sb_extra_id, is_confirmed, confirmed_amount
	  FROM gaming_game_plays FORCE INDEX (sb_extra_id)
	  STRAIGHT_JOIN gaming_payment_transaction_type ON gaming_game_plays.payment_transaction_type_id=gaming_payment_transaction_type.payment_transaction_type_id 
		AND gaming_payment_transaction_type.name IN ('Win','WinPendingAuthorisation')
	  ...
	  SELECT game_round_id, bet_total, bet_total_base, bet_real, bet_cash, bet_bonus, bet_bonus_win_locked, jackpot_contribution, win_total, win_total_base, 
		...
		balance_real_before, balance_bonus_before, loyalty_points, loyalty_points_bonus, round_ref, amount_tax_operator, amount_tax_player, 
        loyalty_points, loyalty_points_bonus
  ...
  SELECT current_real_balance+current_bonus_balance+current_bonus_win_locked_balance AS current_balance, current_real_balance, 
	current_bonus_balance+current_bonus_win_locked_balance AS current_bonus_balance, gaming_currency.currency_code, 
	ROUND(pl_exchange_rate.exchange_rate/gm_exchange_rate.exchange_rate,5) AS exchange_rate, pending_winning_real
  ...
  LEFT JOIN gaming_operator_currency AS gm_exchange_rate ON 
	gaming_operators.operator_id=gm_exchange_rate.operator_id AND gm_currency.currency_id=gm_exchange_rate.currency_id 
  LEFT JOIN gaming_operator_currency AS pl_exchange_rate ON 
	gaming_operators.operator_id=pl_exchange_rate.operator_id AND pl_exchange_rate.currency_id=gaming_currency.currency_id
