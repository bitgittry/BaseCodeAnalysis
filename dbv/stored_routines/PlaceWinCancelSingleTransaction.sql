DROP procedure IF EXISTS `PlaceWinCancelSingleTransaction`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlaceWinCancelSingleTransaction`(roundRef VARCHAR(80), gameSessionID BIGINT, winToCancelAmount DECIMAL(18, 5), cancelTransactionRef VARCHAR(80), transactionRef VARCHAR(80), OUT gamePlayIDReturned BIGINT, OUT statusCode INT)
root:BEGIN
	

	DECLARE gamePlayID, clientStatID, clientID, currencyID, newGamePlayID, gameRoundID, operatorGameID, gamePlayBetCounterID, betGamePlayID, badDeptGamePlayID, sessionID, gameID BIGINT DEFAULT -1;
    ...
    DECLARE usedBonusMoney, disallowNegativeBalance, playLimitEnabled, realMoneyOnly, cancelAlreadyProcessed TINYINT(1) DEFAULT 0;
    DECLARE numBonuses, licenseTypeID, clientWagerTypeID, numTransactions, gameManufacturerID INT DEFAULT 0;

	SET @channelType='online';

	SELECT win.game_play_id, win.client_stat_id, win.amount_real, win.amount_bonus, win.amount_bonus_win_locked, win.game_round_id, 
		  win.operator_game_id, win.currency_id, win.game_play_id, ggr.license_type_id, gg.client_wager_type_id, win.round_transaction_no, 
      IFNULL(win.session_id, win.session_id), gg.game_id, ggr.game_manufacturer_id
	INTO gamePlayID, clientStatID, betRealAmount, betBonusAmount, betBonusWinLockedAmount, gameRoundID, 
		 operatorGameID, currencyID, betGamePlayID, licenseTypeID, clientWagerTypeID, numTransactions, sessionID, gameID, gameManufacturerID
	FROM gaming_game_plays AS win
    STRAIGHT_JOIN gaming_game_rounds AS ggr ON win.game_round_id=ggr.game_round_id AND ggr.round_ref=roundRef
    STRAIGHT_JOIN gaming_games AS gg ON ggr.game_id=gg.game_id
    JOIN gaming_cw_transactions ON gaming_cw_transactions.game_play_id=win.game_play_id 
    AND gaming_cw_transactions.transaction_ref=cancelTransactionRef;       
  ...
  SELECT EXISTS(select * from gaming_cw_transactions 
                      INNER JOIN gaming_payment_transaction_type 
                      ON gaming_cw_transactions.transaction_ref=transactionRef 
                        AND gaming_cw_transactions.payment_transaction_type_id=gaming_payment_transaction_type.payment_transaction_type_id 
                        AND gaming_payment_transaction_type.name='WinCancelled')
    INTO	cancelAlreadyProcessed;
  ...
  IF (cancelAlreadyProcessed=0) THEN
  	...
  	WHERE gcs.client_stat_id=clientStatID;  
  	    ...
  	    INSERT INTO gaming_cw_transactions (game_manufacturer_id, payment_transaction_type_id, amount, transaction_ref, round_ref, game_ref, 
        client_stat_id, game_play_id, timestamp, other_data, is_success, status_code)
      SELECT gameManufacturerID, transaction_type.payment_transaction_type_id, winToCancelAmount, transactionRef, roundRef, roundRef, 
      clientStatID, NULL, NOW(), NULL, IF(statusCode=50,1,0), statusCode 
      FROM gaming_payment_transaction_type AS transaction_type WHERE transaction_type.name='WinCancelled';
