DROP procedure IF EXISTS `PlayerGetSessionTrail`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlayerGetSessionTrail`(clientStatID BIGINT, sessionID BIGINT)
BEGIN
  -- will trieve all transaction with sessionID	
  ...
  IF (sportsBookActive=0 AND poolBettingActive=0 AND lotteryActive=0 AND sportsPoolActive=0) THEN
		...
		FROM gaming_game_plays FORCE INDEX (session_id)
		...
		JOIN gaming_payment_transaction_type ON 
		  gaming_game_plays.session_id=sessionID AND (gaming_game_plays.timestamp BETWEEN @date_from AND @date_to) AND
  ...
  ELSE
		...
		-- Lottery/SportsPool
		LEFT JOIN gaming_lottery_coupons ON gaming_game_plays.sb_bet_id=gaming_lottery_coupons.lottery_coupon_id AND gaming_license_type.license_type_id IN (6, 7)
