-- -------------------------------------
-- BonusGetForPromotionRuleByID.sql
-- -------------------------------------
DROP procedure IF EXISTS `BonusGetForPromotionRuleByID`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `BonusGetForPromotionRuleByID`(bonusRuleID BIGINT)
BEGIN
	...
	SELECT gaming_operator_games.game_id, gaming_operator_games.operator_game_id,  
	...
	SELECT gaming_bonus_rules_for_promotions_amounts.bonus_rule_id, amount AS forpromotion_amount, gaming_currency.currency_id, currency_code 
	FROM gaming_bonus_rules_for_promotions_amounts
	JOIN gaming_currency ON gaming_bonus_rules_for_promotions_amounts.currency_id=gaming_currency.currency_id
	WHERE gaming_bonus_rules_for_promotions_amounts.bonus_rule_id = bonusRuleID;
END$$
