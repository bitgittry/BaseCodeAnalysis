DROP procedure IF EXISTS `RuleEngineProcessTransactionReturnSB`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionReturnSB`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  DECLARE cur1 CURSOR FOR 
    SELECT gaming_rules.rule_id, gaming_game_plays.client_stat_id, gaming_game_plays.amount_total_base, 
        ...
        FROM 
          ...
          CROSS JOIN (SELECT gaming_game_plays_sb.client_stat_id, gaming_game_plays_dvtbl.amount_total_base, gaming_game_plays_sb.game_play_sb_id, gaming_game_plays_sb.timestamp  
            FROM (SELECT * FROM gaming_game_plays WHERE game_play_id=transactionId AND license_type_id=3 AND payment_transaction_type_id=13) gaming_game_plays_dvtbl 
  ...
  read_loop: LOOP
    ...
    SET logTxt=CONCAT(logTxt, '//[RETURN_SB] RULE_', ruleId);
    IF dateCheck=0 THEN
        ...
        IF ruleInstanceId IS NOT NULL THEN
          ...
          INNER JOIN (
           SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                    @gamePlayCount:=@gamePlayCount + CASE WHEN criteria_name='ReturnNumPlays' THEN 1 ELSE 0 END,
                   @logTxt2:=CONCAT(@logTxt2,' EV_', dvtbl2.event_id, '_', criteria_name,'+', case when criteria_name='ReturnSum' THEN varAmount 
                        ELSE CASE WHEN criteria_name='ReturnNumRounds' THEN (SELECT COUNT(game_play_sb_id) FROM gaming_game_plays_sb WHERE game_play_id=transactionId)
                        ELSE 1 END END)
           ...
           SELECT event_id,  @logTxt:=CONCAT(@logTxt, GROUP_CONCAT(CASE WHEN res=1 THEN '' ELSE CONCAT(' EV_', event_id, '_', criteria_name,'=FAILED') END SEPARATOR '')),
            ...
            case when 
              (criteria_name='StakeSBProfileID' and 
                (
                  ...
                  OR (operator='not_in' AND fnRuleEngineIsBetInSbProfile(gamingGamePlaysSbId, filter_csv)=0)
                )
              )
            then 1 else   
            ...
            JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
              AND gaming_re_event_criteria.event_type_id=3 AND gaming_re_event_criteria.criteria_name not in ('ReturnVertical','ReturnGame','ReturnGameCategory')
            ...
            SET incr_result=IFNULL(incr_result,0) + 
              ...
              ELSE CASE WHEN criteria_name='ReturnNumRounds' THEN 1
