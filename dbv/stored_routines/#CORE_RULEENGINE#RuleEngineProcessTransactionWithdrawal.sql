DROP procedure IF EXISTS `RuleEngineProcessTransactionWithdrawal`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionWithdrawal`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  -- the input transaction (WITHDRAWAL) is checked against all the criteria that could be affected by it.
  ...
  DECLARE cur1 CURSOR FOR 
    SELECT gaming_rules.rule_id, gaming_balance_history.client_stat_id, gaming_balance_history.amount_base, 
            ...
            gaming_rules.player_max_occurrences, gaming_balance_history.request_timestamp,
            CASE WHEN (gaming_balance_history.request_timestamp between IFNULL(gaming_rules.start_date,'2001-01-01') and IFNULL(gaming_rules.end_date,'2399-01-01')) THEN 1 ELSE 0 END as date_check,
            CASE WHEN ((IFNULL(gaming_rules.player_selection_id,0)=0) OR EXISTS (SELECT player_selection_id FROM gaming_player_selections_player_cache 
                    WHERE client_stat_id=gaming_balance_history.client_stat_id AND player_selection_id =IFNULL(gaming_rules.player_selection_id, 0) AND player_in_selection=1)) THEN 1 ELSE 0 END as player_selection_check            
        FROM 
          (SELECT gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, gaming_rules.max_occurrences, gaming_rules.player_max_occurrences, 
            ...
            WHERE gaming_events.event_type_id=6 AND is_active=1 AND is_hidden=0 GROUP BY gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, 
          ...
          JOIN (SELECT * FROM gaming_balance_history WHERE payment_transaction_type_id = 2 AND balance_history_id=transactionId) as gaming_balance_history
          LEFT JOIN gaming_rules_instances on gaming_rules.rule_id=gaming_rules_instances.rule_id AND gaming_rules_instances.is_current=1 AND gaming_rules_instances.is_achieved=0 AND 
            gaming_rules_instances.client_stat_id=gaming_balance_history.client_stat_id;
  ...
  read_loop: LOOP
    SET done=FALSE;
    FETCH cur1 INTO ruleId, clientId, varAmount, ruleInstanceId, achievementIntervalTypeId, amountAchieved, maxoccurrences, maxoccurrencesPlayer, transactionDate, dateCheck, playerSelectionCheck;
    IF done THEN 
      LEAVE read_loop;
    END IF;
    SET logTxt=CONCAT(logTxt, '//[WITHDRAWAL] RULE_', ruleId);
    IF dateCheck=0 THEN
    ...
    ELSE
      ...
      ELSE
        -- SELECT ruleId, clientId, varAmount, ruleInstanceId, achievementIntervalTypeId, amountAchieved, maxoccurrences, amountAchievedPlayer, maxoccurrencesPlayer, transactionDate;
        ...
        ELSE
          ...
          -- creates a new istance only if the option "Enable One Award Achievement per Interval" is satisfied
            ...
            -- and the total number awarded to the player till now is less than the max_occurrences available for each player
        ...
        IF ruleInstanceId IS NOT NULL THEN
          ...
          SELECT ruleInstanceId, event_id, clientId, 1 FROM (
          ...
          -- updates totals and sums (statuses) when all the conditions are verified
          ...
          INNER JOIN (
           SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                   @logTxt2:=CONCAT(@logTxt2,' EV_', dvtbl2.event_id, '_', criteria_name,'+',case when criteria_name='WithdrawalSum' THEN varAmount ELSE 1 END)
                 FROM (
           ...
           CASE WHEN count(res) = sum(res) THEN 1 ELSE 0 END AS res FROM (
            ...
            case when 
              (criteria_name='WithdrawalSize' and 
                (
                  (operator='equal' and amount_base = lower_bound)
                  OR (operator='greater' and amount_base > lower_bound)
                  OR (operator='greater_or_equal' and amount_base >= lower_bound)
                  OR (operator='less' and amount_base < lower_bound)
                  OR (operator='less_or_equal' and amount_base <= lower_bound)
                  OR (operator='between' and amount_base between lower_bound and upper_bound)
                )
              )
            then 1 else 
              
            case when 
              (criteria_name='WithdrawalCurrency' and 
                (
                  (operator='in' AND CONCAT(',',filter_csv,',') LIKE CONCAT('%,',CAST(gaming_balance_history.currency_id AS CHAR),',%'))
                  OR (operator='not_in' AND CONCAT(',',filter_csv,',') NOT LIKE CONCAT('%,',CAST(gaming_balance_history.currency_id AS CHAR),',%'))
                )
              )
            then 1 else 

            case when 
              (criteria_name='WithdrawalDate' and 
                (
                  (operator='on' and datediff(gaming_balance_history.request_timestamp, filter_date_from)=0)
                  OR (operator='between' and gaming_balance_history.request_timestamp between filter_date_from and filter_date_to)
                  OR (operator='until' and datediff(gaming_balance_history.request_timestamp, filter_date_from)<=0)
                )
              )
            then 1 else 
            
            0 END END END END as res
            from         
            gaming_balance_history 
            join gaming_rules_instances ON gaming_balance_history.balance_history_id=transactionId AND payment_transaction_type_id = 2  AND rule_instance_id=ruleInstanceId
            ...
            join gaming_re_event_criteria on gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
              AND gaming_re_event_criteria.event_type_id=6) dvtbl
          ...
          JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
           AND dvtbl2.res=1 AND gaming_re_event_criteria.criteria_name in ('WithdrawalSum','WithdrawalCount')
          JOIN gaming_re_event_criteria_instances ON gaming_re_event_criteria_instances.re_event_criteria_config_id=gaming_re_event_criteria_config.re_event_criteria_config_id
           AND gaming_re_event_criteria_instances.client_stat_id=clientId AND gaming_re_event_criteria_instances.rule_instance_id=ruleInstanceId
           ) dvtbl3
                 ON
                    dvtbl3.re_event_criteria_instance_id= gaming_re_event_criteria_instances.re_event_criteria_instance_id
          SET incr_result=IFNULL(incr_result,0)+case when criteria_name='WithdrawalSum' THEN varAmount ELSE 1 END;
            ...
            IF (isAchieved=1) THEN
              UPDATE gaming_rules_instances SET is_achieved=isAchieved, end_date=NOW(), is_current=0 WHERE is_achieved=0 AND rule_instance_id=ruleInstanceId;
  ...
  IF (isLogActive=1) AND (clientId IS NOT NULL) THEN 
    UPDATE gaming_event_rows SET client_id=clientId, `log`=CONCAT(`log`,' ',logTxt) WHERE event_table_id=2 AND elem_id=transactionId; 
  END IF;
END root$$
