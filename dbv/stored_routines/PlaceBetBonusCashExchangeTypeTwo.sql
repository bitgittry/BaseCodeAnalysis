DROP procedure IF EXISTS `PlaceBetBonusCashExchangeTypeTwo`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlaceBetBonusCashExchangeTypeTwo`(clientStatID BIGINT, gamePlayID BIGINT, sessionID BIGINT, transactionType VARCHAR(45), exchangeRate DECIMAL(18,5), bonusTransferedTotal DECIMAL(18,5), bonusTransfered DECIMAL(18,5), bonusWinLockedTransfered DECIMAL(18,5), bonusTransferedLost DECIMAL(18,5), bonusWinLockedTransferedLost DECIMAL(18,5),
...
IF (FreeBonusAmount IS NULL AND transactionType = 'CashBonus') THEN
 -- if the bonus is cachBonus and there is no wagering (is_free_bonus = 1), the bonus was credit to FreeBet and now needs to be Debit from FreeBet too.
