DROP procedure IF EXISTS `CommonWalletSportsGenericCalculateBonusRuleWeight`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletSportsGenericCalculateBonusRuleWeight`(
...
BEGIN
  ...
  -- For all singles check which bonus rules (and by definition which bonus instances) 
  -- are applicable and get weights of all eligible selections
  
  IF (numSingles>0) THEN
	  INSERT INTO gaming_sb_bets_bonus_rules (sb_bet_id, bonus_rule_id, weight) 
		SELECT DISTINCT sbBetID, inst.bonus_rule_id, /*MIN(*/ IFNULL(wght.weight, 1) /*)*/
		...
		STRAIGHT_JOIN gaming_bonus_instances AS inst ON inst.client_stat_id = clientStatID AND inst.is_active = 1
		STRAIGHT_JOIN gaming_bonus_rules AS rul ON inst.bonus_rule_id = rul.bonus_rule_id
		STRAIGHT_JOIN gaming_bonus_rules_wgr_sb_eligibility_criterias AS crit ON inst.bonus_rule_id = crit.bonus_rule_id
		...
		STRAIGHT_JOIN gaming_bonus_rules_wgr_sb_weights AS wght 
			...
			AND (rul.min_odd IS NULL OR sing.odd >= rul.min_odd)
		LEFT JOIN sessions_main AS sess ON sess.session_id = sessionID
		LEFT JOIN gaming_bonus_rules_platform_types AS platypes ON 
			rul.bonus_rule_id = platypes.bonus_rule_id AND sess.platform_type_id = platypes.platform_type_id
		WHERE sing.sb_bet_id = sbBetID AND 
			-- parition filtering
			(sing.sb_bet_single_id BETWEEN minSbBetSingleID AND maxSbBetSingleID) AND
            -- other filtering
            (rul.restrict_platform_type = 0 OR platypes.platform_type_id IS NOT NULL)
		GROUP BY inst.bonus_instance_id
		HAVING COUNT(DISTINCT sing.sb_bet_single_id) = numSingles
		ON DUPLICATE KEY UPDATE weight=LEAST(weight, VALUES(weight));
  END IF;

  -- For all multiples check which bonus rules (and by definition which bonus instances) 
  -- are applicable and get weights of all eligible selections per accumulators, simple systems and full systems
  IF (numMultiples>0) THEN
    ...
    SELECT DISTINCT sbBetID, mult_wght.bonus_rule_id, 1, /*MIN(*/ IFNULL(mult_wght.weight, 1) /*)*/
  	FROM
  	(
  		SELECT mul.sb_bet_multiple_id, inst.bonus_rule_id, 
			COUNT(DISTINCT mulsin.sb_bet_multiple_single_id) AS num_singles, 
			wght.min_odd AS weight_min_odd, wght.max_odd AS weight_max_odd, wght.weight AS weight, rul.min_odd AS bonus_rule_min_odd
          ...
          ON wsel4.eligibility_criterias_id = wsel5.eligibility_criterias_id AND wsel5.sb_entity_type_id = 5 AND (wsel5.sb_entity_id IS NULL OR wsel5.sb_entity_id = sel.sb_market_id)
  		STRAIGHT_JOIN gaming_bonus_rules_wgr_sb_weights AS wght 
          ON wsel5.eligibility_criterias_id = wght.eligibility_criterias_id OR inst.bonus_rule_id = wght.bonus_rule_id
      LEFT JOIN sessions_main AS sess ON sess.session_id = sessionID
      LEFT JOIN gaming_bonus_rules_platform_types AS platypes ON rul.bonus_rule_id = platypes.bonus_rule_id AND sess.platform_type_id = platypes.platform_type_id
  		WHERE mul.sb_bet_id = sbBetID
		  -- parition filtering
		  AND (mul.sb_bet_multiple_id BETWEEN minSbBetMultipleID AND maxSbBetMultipleID)
		  -- other filtering
          ...
          AND (rul.restrict_platform_type = 0 OR platypes.platform_type_id IS NOT NULL)   
  		GROUP BY mul.sb_bet_multiple_id, inst.bonus_rule_id, wght.min_odd
  	) AS mult_wght 
