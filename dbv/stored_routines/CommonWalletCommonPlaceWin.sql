DROP procedure IF EXISTS `CommonWalletCommonPlaceWin`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletCommonPlaceWin`(
  gameSessionKey VARCHAR(40), clientStatID BIGINT, gameManufacturerName VARCHAR(80), transactionRef VARCHAR(80), roundRef BIGINT, 
  gameRef VARCHAR(80), winAmount DECIMAL(18, 5), varReason VARCHAR(80), canCommit TINYINT(1), 
...
root: BEGIN
  ...
  DECLARE clearBonusLost, cwCloseRoundOnWin, cwNoRound, isAlreadyProcessed, jackpotWinMoneyFlowAsNormalWin TINYINT(1) DEFAULT 0; 
  ...
  DECLARE ignoreSessionExpiry, extendSessionExpiry, isJackpotWin, closeRound TINYINT(1) DEFAULT 0;
  ...
  SELECT gaming_game_sessions.game_id, gaming_game_sessions.session_id, gaming_game_sessions.client_stat_id, gaming_game_sessions.game_manufacturer_id, cw_close_round_onwin, cw_exchange_currency, cw_no_round 
  INTO gameID, sessionID, clientStatID, gameManufacturerID, cwCloseRoundOnWin, cwExchangeCurrency, cwNoRound
  ...
  IF (IFNULL(roundRef,0)=0 AND cwNoRound) THEN
    SELECT cw_round_id INTO roundRef FROM gaming_cw_rounds 
    WHERE client_stat_id=clientStatID AND game_id=gameID AND cw_latest;
  END IF;
