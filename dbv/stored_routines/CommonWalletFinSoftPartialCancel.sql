DROP procedure IF EXISTS `CommonWalletFinSoftPartialCancel`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletFinSoftPartialCancel`(clientStatID BIGINT, transactionRef VARCHAR(50), betRef VARCHAR(40), adjustAmount DECIMAL(18,5), canCommit TINYINT(1), OUT statusCode INT)
root: BEGIN
  
  
  DECLARE gameManufacturerName VARCHAR(20) DEFAULT 'FinSoft';
  DECLARE gameManufacturerID,x BIGINT DEFAULT 7; 
  DECLARE sbBetWinID, gamePlayID, sbBetID, sbExtraID, clientStatIDCheck, clientID, currencyID, sbBetIDCheck, gamePlayMessageTypeID, sessionID, countryID BIGINT DEFAULT -1; 
  ...
  END IF;
	...
	INTO betTotal
	FROM gaming_game_plays WHERE game_round_id=gameRoundID AND  payment_Transaction_type_id IN (45,12);
	...
	IF (@numPlayBonusInstances>0) THEN
		...
		(
		  SELECT 
			...
			ROUND((SUM(bet_real)/betTotal)*adjustAmount,0) + IF (gaming_bonus_instances.is_secured,ROUND((SUM(bet_bonus+bet_bonus_win_locked)/betTotal)*adjustAmount, 0) ,0)  AS bet_real
  ...
  IF (taxEnabled) THEN
    ...
    IF (roundWinTotal != 0.0 OR taxAlreadyChargedOperator != 0.0 OR taxAlreadyChargedPlayer != 0.0) THEN
		...
		IF (countryID > 0) THEN
		  ...
		  SELECT bet_tax, win_tax, apply_net_deduction, tax_paid_by_operator_win INTO taxBet, taxWin, applyNetDeduction, winTaxPaidByOperator
		  FROM gaming_country_tax AS gct
		  WHERE gct.country_id = countryID AND gct.is_current =  1 AND gct.licence_type_id = licenseTypeID LIMIT 1;
		
		  SET roundBetTotalReal = roundBetTotalReal + adjustReal;
		  SET roundWinTotalFull = roundWinTotalReal; 

			
		  IF(roundWinTotalFull > roundBetTotalReal) THEN
  ...
  SET taxModificationPlayer = amountTaxPlayer - taxAlreadyChargedPlayer;
	...
	JOIN gaming_client_stats ON gaming_payment_transaction_type.name='PartialCancel' AND gaming_client_stats.client_stat_id=clientStatID;
	...
	FROM gaming_sb_bet_transaction_types WHERE name='PartialCancel';
