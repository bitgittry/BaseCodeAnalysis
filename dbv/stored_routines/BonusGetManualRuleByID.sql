-- -------------------------------------
-- BonusGetManualRuleByID.sql
-- -------------------------------------
DROP procedure IF EXISTS `BonusGetManualRuleByID`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `BonusGetManualRuleByID`(bonusRuleID BIGINT)
BEGIN
	...
	SELECT gaming_bonus_rules.bonus_rule_id, gaming_bonus_rules.name, gaming_bonus_rules.description, gaming_bonus_types.name AS bonus_type_name, priority, activation_start_date, activation_end_date, program_cost_threshold,
		...
		min_odd, withdrawal_limit_num_rounds, over_max_bet_win_contr_multiplier, casino_weight_mod, poker_weight_mod, sportsbook_weight_mod, poolbetting_weight_mod, lottery_weight_mod, sportspool_weight_mod, gaming_bonus_rules.restrict_platform_type, gaming_bonus_rules.forfeit_on_withdraw,
	...
	WHERE gaming_bonus_rules_manuals_amounts.bonus_rule_id=bonusRuleID;
END$$
