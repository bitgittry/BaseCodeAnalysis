DROP procedure IF EXISTS `CommonWalletGetBalanceWithGame`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletGetBalanceWithGame`(gameManufacturerName VARCHAR(80), clientStatID BIGINT, gameManufacturerIDF VARCHAR(80))
BEGIN
	...
	JOIN gaming_games ON gaming_games.manufacturer_game_idf=gameManufacturerIDF AND gaming_game_manufacturers.name=gameManufacturerName AND gaming_game_manufacturers.game_manufacturer_id=gaming_games.game_manufacturer_id
	...
	IF(cw_freerounds_addtobonus,IFNULL(FreeRounds.free_rounds_balance,0),0) AS current_balance, current_real_balance, 
	...
	JOIN gaming_games ON gaming_games.manufacturer_game_idf=gameManufacturerIDF AND gaming_games.game_manufacturer_id=gaming_game_manufacturers.game_manufacturer_id
	...
	(
		...
		FROM gaming_cw_free_rounds AS gcwfr
		JOIN gaming_client_stats 
			ON gaming_client_stats.client_stat_id = clientStatID 
			AND gcwfr.client_stat_id = gaming_client_stats.client_stat_id 		
		JOIN gaming_games 
			ON gaming_games.manufacturer_game_idf=gameManufacturerIDF 
		JOIN gaming_bonus_rules 
			ON gcwfr.bonus_rule_id = gaming_bonus_rules.bonus_rule_id
		JOIN gaming_bonus_instances 
			ON gaming_bonus_instances.bonus_rule_id = gaming_bonus_rules.bonus_rule_id
			AND gaming_bonus_instances.cw_free_round_id = gcwfr.cw_free_round_id
		...
		JOIN gaming_bonus_free_round_profiles 
			ON gaming_bonus_rule_free_round_profiles.bonus_free_round_profile_id = gaming_bonus_free_round_profiles.bonus_free_round_profile_id 
			AND gaming_bonus_free_round_profiles.is_active = 1
			AND gaming_bonus_free_round_profiles.is_hidden = 0
			AND gaming_bonus_free_round_profiles.game_manufacturer_id = gaming_games.game_manufacturer_id	
		JOIN gaming_bonus_free_round_profiles_games 
			ON gaming_bonus_free_round_profiles_games.bonus_free_round_profile_id = gaming_bonus_free_round_profiles.bonus_free_round_profile_id
			AND gaming_bonus_free_round_profiles_games.game_id = gaming_games.game_id		
		JOIN gaming_cw_free_round_statuses gcwfrs ON gcwfr.cw_free_round_status_id = gcwfrs.cw_free_round_status_id 
			AND gcwfrs.name NOT IN ('FinishedAndTransfered','Forfeited','Expired')	
