DROP procedure IF EXISTS `TransactionAdjustRealMoneyAllowNegative`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `TransactionAdjustRealMoneyAllowNegative`(sessionID BIGINT, clientStatID BIGINT, varAmount DECIMAL(18, 5), varDescription TEXT, transactionType VARCHAR(40), transactionKey VARCHAR(80), OUT statusCode INT)
root: BEGIN
...
IF @varAmount>0 AND adjustmentSelector NOT IN ('B','G') THEN
  ...
  SELECT gaming_payment_transaction_type.payment_transaction_type_id, @varAmount, ROUND(@varAmount/exchangeRate,5), gaming_client_stats.currency_id, exchangeRate, @varAmount, 0, 0, 0, NOW(), gaming_client_stats.client_id, gaming_client_stats.client_stat_id, current_real_balance, current_bonus_balance, current_bonus_win_locked_balance, current_loyalty_points, sessionID, sessionID, @description, pending_bets_real, pending_bets_bonus,withdrawal_pending_amount,0,(gaming_client_stats.`total_loyalty_points_given_bonus` - gaming_client_stats.`total_loyalty_points_used_bonus`) 
  FROM gaming_client_stats 
  ...
  SELECT transaction_id, gaming_payment_transaction_type.payment_transaction_type_id AS transaction_type_id, gaming_payment_transaction_type.name AS transaction_type_name, 
    ...
    gaming_transactions.timestamp, gaming_transactions.exchange_rate, gaming_transactions.client_stat_id, balance_real_after, balance_bonus_after, balance_bonus_win_locked_after, loyalty_points_after, 
    gaming_transactions.reason, gaming_currency.currency_code, gaming_transactions.balance_history_id
  FROM gaming_transactions
