DROP procedure IF EXISTS `ReturnFundsTypeTwoLotto`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `ReturnFundsTypeTwoLotto`(
  couponID BIGINT, gamePlayID BIGINT, sessionID BIGINT, OUT statusCode INT)
root : BEGIN
    -- Added gaming_game_play_message_types
    -- Changed message type: LotteryBetCancelled 
    -- Play Limit Update called per game
    -- Fixed bug when bonus was lost or secured in that gaming_bonus_instances was not updated but in gaming_client_stats the funds were being added incorrectly
	-- Optimizations: Forcing STRAIGHT_JOINS and INDEXES 
    -- Merged from INLW

	DECLARE tempGamePlayID, gameRoundID, gameManufacturerID, newGamePlayID, clientStatID, platformTypeID, clientID, currencyID BIGINT(20);
	DECLARE betAmount,exchangeRate, betReal, betBonus, betBonusWinLocked, totalLoyaltyPoints, totalLoyaltyPointsBonus, cancelBonus, cancelBonusWinLocked, releasedLockedFunds DECIMAL(18,5);
	...
	INTO playLimitEnabled, bonusEnabledFlag, ringFencedEnabled /*to be implemented in a later phase*/
	FROM gaming_settings gs1 
	JOIN gaming_settings gs2 ON (gs2.name='IS_BONUS_ENABLED')
	LEFT JOIN gaming_settings gs3 ON (gs3.name='RING_FENCED_ENABLED')
	WHERE gs1.name='PLAY_LIMIT_ENABLED';

	SELECT client_stat_id, game_manufacturer_id, lottery_wager_status_id, error_code, gaming_lottery_coupons.license_type_id, gaming_license_type.`name`, gaming_client_wager_types.client_wager_type_id  
	...
	IF (gamePlayID = 0) THEN 
        ...
        amount_bonus_win_locked, gaming_game_plays.loyalty_points, gaming_game_plays.loyalty_points_bonus, gaming_game_plays.game_manufacturer_id, gaming_game_plays.session_id, gaming_game_plays.released_locked_funds
		INTO tempGamePlayID, gameRoundID, clientStatID, betReal, platformTypeID, betBonus, betBonusWinLocked, totalLoyaltyPoints, totalLoyaltyPointsBonus, gameManufacturerID, sessionID, releasedLockedFunds
    ...
    ELSE
		...
		SELECT game_round_id, client_stat_id, amount_real, platform_type_id, amount_bonus, amount_bonus_win_locked, loyalty_points, loyalty_points_bonus, game_manufacturer_id, gaming_game_plays.session_id, gaming_game_plays.released_locked_funds
        INTO gameRoundID, clientStatID, betReal, platformTypeID, betBonus, betBonusWinLocked, totalLoyaltyPoints, totalLoyaltyPointsBonus, gameManufacturerID, sessionID, releasedLockedFunds
    ...
    -- 8 = Cancelled - Added for Chile to Cancel Coupons for extraordinary events and also Return Funds to the Players
    IF (wagerStatusCode != 8) THEN
      -- If Wager Status Code is FundsReserved Skip Validation
      IF (wagerStatusCode != 3) THEN
        -- Wager Status Code 4 = FundsReturned. We Return as it's already processed 
        IF (wagerStatusCode = 4) THEN

          CALL PlayReturnDataWithoutGame(IFNULL(gamePlayID, 0), IFNULL(gameRoundID, 0), clientStatID, gameManufacturerID, 0);
          CALL PlayReturnBonusInfoOnWin(IFNULL(gamePlayID, 0));
    ...
    STRAIGHT_JOIN gaming_lottery_participations ON gaming_lottery_participations.lottery_dbg_ticket_id = gaming_lottery_dbg_tickets.lottery_dbg_ticket_id AND (gaming_lottery_participations.lottery_wager_status_id = 3 /*requires get funds*/  
	OR gaming_lottery_participations.lottery_wager_status_id = 5) -- Temporary Chile Hot Fix for INBUGCL-325 
    WHERE gaming_lottery_dbg_tickets.lottery_coupon_id = couponID;
    
	UPDATE gaming_lottery_coupons
    SET gaming_lottery_coupons.lottery_coupon_status_id = IF (gaming_lottery_coupons.lottery_wager_status_id = 3,2112,2104),
		gaming_lottery_coupons.lottery_wager_status_id = 4
    WHERE gaming_lottery_coupons.lottery_coupon_id = couponID;
    
	UPDATE gaming_lottery_dbg_tickets FORCE INDEX (lottery_coupon_id)
    STRAIGHT_JOIN gaming_lottery_participations ON gaming_lottery_participations.lottery_dbg_ticket_id = gaming_lottery_dbg_tickets.lottery_dbg_ticket_id
		AND gaming_lottery_participations.lottery_wager_status_id = 3
	SET 
		gaming_lottery_participations.lottery_participation_status_id = IF(gaming_lottery_participations.lottery_wager_status_id = 3, 2112, 2103),
		gaming_lottery_participations.lottery_wager_status_id = 4
    WHERE gaming_lottery_dbg_tickets.lottery_coupon_id = couponID;
    
	/*																	
    UPDATE gaming_lottery_coupons
    SET gaming_lottery_coupons.lottery_wager_status_id = 4, gaming_lottery_coupons.lottery_coupon_status_id = 2104
    WHERE gaming_lottery_coupons.lottery_coupon_id = couponID;
	*/
	...
	INTO cancelBonus, cancelBonusWinLocked
	...
	SET
	  ...
	  gaming_game_rounds.balance_bonus_after = gaming_client_stats.current_bonus_balance+gaming_client_stats.current_bonus_win_locked_balance +  betBonus + betBonusWinLocked - cancelBonus - cancelBonusWinLocked,
	...
	JOIN gaming_game_rounds FORCE INDEX (PRIMARY) ON gaming_game_rounds_lottery.game_round_id=gaming_game_rounds.game_round_id
    JOIN gaming_client_stats FORCE INDEX (PRIMARY) ON gaming_game_rounds.client_stat_id = gaming_client_stats.client_stat_id
	SET
	  gaming_game_rounds.bet_total = 0,
	  gaming_game_rounds.bet_total_base = 0,
	...
	INSERT INTO gaming_game_plays 
		...
		timestamp, game_manufacturer_id, client_id, client_stat_id, payment_transaction_type_id, balance_real_after, balance_bonus_after, balance_bonus_win_locked_after, pending_bet_real, pending_bet_bonus, 
		 currency_id, sign_mult, license_type_id,loyalty_points, loyalty_points_bonus,loyalty_points_after, loyalty_points_after_bonus, sb_bet_id, game_play_message_type_id, is_cancelled) 
	SELECT ggp.amount_total, ggp.game_round_id, ggp.amount_total/exchangeRate, exchangeRate, ggp.amount_real, ggp.amount_bonus-cancelBonus, ggp.amount_bonus_win_locked-cancelBonusWinLocked, ggp.amount_free_bet, 0, cancelBonus, cancelBonusWinLocked, 
	...
	SET 
		...
		is_used_all=IF(is_active=1 AND gaming_bonus_instances.open_rounds-numParticpations<=0 AND is_used_all=0 AND is_lost=0 
	...
	SET 
        ...
        open_rounds = gaming_bonus_instances.open_rounds-numParticpations
	WHERE gaming_game_plays_bonus_instances.game_play_id=gamePlayID;
	...
	gcs.total_real_played = IF(@channelType NOT IN ('online','retail','self-service'),gcs.total_real_played-betReal, gcs.total_wallet_real_played + gcs.total_cash_played),
		...
		current_bonus_balance=current_bonus_balance+betBonus-cancelBonus, current_bonus_win_locked_balance=current_bonus_win_locked_balance+betBonusWinLocked-cancelBonusWinLocked, 
    ...
    CALL PlayerUpdateVIPLevel(clientStatID,1);
