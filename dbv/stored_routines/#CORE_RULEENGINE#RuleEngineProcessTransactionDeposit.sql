DROP procedure IF EXISTS `RuleEngineProcessTransactionDeposit`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionDeposit`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  -- the input transaction (DEPOSIT) is checked against all the criteria that could be affected by it.
    ...
    SELECT gaming_rules.rule_id, gaming_balance_history.client_stat_id, gaming_balance_history.amount_base, 
        ...
        FROM 
          (SELECT gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, gaming_rules.max_occurrences, gaming_rules.player_max_occurrences, 
            ...
            WHERE gaming_events.event_type_id=1 AND is_active=1 AND is_hidden=0 GROUP BY gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, 
          ...
          CROSS JOIN (SELECT * FROM gaming_balance_history WHERE payment_transaction_type_id = 1 AND balance_history_id=transactionId) as gaming_balance_history
  ...
  read_loop: LOOP
    ...
    IF done THEN 
      LEAVE read_loop;
    END IF;
    SET logTxt=CONCAT(logTxt, '//[DEPOSIT] RULE_', ruleId);
    IF dateCheck=0 THEN
        ...
        IF ruleInstanceId IS NOT NULL THEN
          ...
          INNER JOIN (
           SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                   @logTxt2:=CONCAT(@logTxt2,' EV_', dvtbl2.event_id, '_', criteria_name,'+',case when criteria_name='DepositSum' THEN varAmount ELSE 1 END)
           FROM (
           ...
           SELECT event_id,  @logTxt:=CONCAT(@logTxt, GROUP_CONCAT(CASE WHEN res=1 THEN '' ELSE CONCAT(' EV_', event_id, '_', criteria_name,'=FAILED') END SEPARATOR '')),
            ...
            case when 
              (criteria_name='DepositCurrency' and 
                (
            ...
            case when 
              (criteria_name='DepositPaymentMethod' and 
                (
                  (operator='in' AND CONCAT(',',filter_csv,',') LIKE CONCAT('%,',CAST(gaming_balance_history.sub_payment_method_id AS CHAR),',%'))
                  OR (operator='not_in' AND CONCAT(',',filter_csv,',') NOT LIKE CONCAT('%,',CAST(gaming_balance_history.sub_payment_method_id AS CHAR),',%'))
                )
              )
            then 1 else 

            case when 
              (criteria_name='DepositDate' and 
                (
            ...
            join gaming_rules_instances ON gaming_balance_history.balance_history_id=transactionId AND payment_transaction_type_id = 1  AND rule_instance_id=ruleInstanceId
            ...
            join gaming_re_event_criteria on gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
              AND gaming_re_event_criteria.event_type_id=1
            ...
            JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
             AND dvtbl2.res=1 AND gaming_re_event_criteria.criteria_name in ('DepositSum','DepositCount')
            ...
            SET incr_result=IFNULL(incr_result,0)+case when criteria_name='DepositSum' THEN varAmount ELSE 1 END;
