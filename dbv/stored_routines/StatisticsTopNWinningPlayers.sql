DROP procedure IF EXISTS `StatisticsTopNWinningPlayers`;
...
BEGIN
  ...
  FROM
  (
    SELECT @rowNum:=@rowNum+1 AS row_num, gaming_clients.nickname, gaming_currency.currency_code, gaming_countries.country_code AS country_code, GG.win_total  
    ...
    (
      ...
      FROM sessions_main
      JOIN gaming_client_sessions ON sessions_main.date_open BETWEEN @dateFrom AND @dateTo AND sessions_main.session_id=gaming_client_sessions.session_id AND ((total_win-total_bet)>=1000)
      ORDER BY sessions_main.date_open DESC
