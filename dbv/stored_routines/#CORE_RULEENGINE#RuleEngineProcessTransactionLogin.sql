DROP procedure IF EXISTS `RuleEngineProcessTransactionLogin`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionLogin`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  -- the input transaction (LOGIN) is checked against all the criteria that could be affected by it.
  ...
  DECLARE cur1 CURSOR FOR 
    SELECT gaming_rules.rule_id, sessions_main.extra2_id, timestampdiff(minute, IFNULL(sessions_main.date_open,now()), IFNULL(sessions_main.date_closed,now())) as sessionduration, 
            ...
            gaming_rules.player_max_occurrences, sessions_main.date_open,
            CASE WHEN (sessions_main.date_open between IFNULL(gaming_rules.start_date,'2001-01-01') and IFNULL(gaming_rules.end_date,'2399-01-01')) THEN 1 ELSE 0 END as date_check,
              CASE WHEN ((IFNULL(gaming_rules.player_selection_id,0)=0) OR EXISTS (SELECT player_selection_id FROM gaming_player_selections_player_cache 
                WHERE client_stat_id=sessions_main.extra2_id AND player_selection_id =IFNULL(gaming_rules.player_selection_id, 0) AND player_in_selection=1)) THEN 1 ELSE 0 END as player_selection_check 
        FROM 
          (SELECT gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, gaming_rules.max_occurrences, gaming_rules.player_max_occurrences, 
            ...
            WHERE gaming_events.event_type_id=4 AND is_active=1 AND is_hidden=0 GROUP BY gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, 
          ...
          CROSS JOIN (SELECT * FROM sessions_main WHERE session_id=transactionId) as sessions_main
          LEFT JOIN gaming_rules_instances on gaming_rules.rule_id=gaming_rules_instances.rule_id AND gaming_rules_instances.is_current=1 
              AND gaming_rules_instances.is_achieved=0 AND 
            gaming_rules_instances.client_stat_id=sessions_main.extra2_id;
  ...
  read_loop: LOOP
    ...
    IF done THEN 
      LEAVE read_loop;
    END IF;
    SET logTxt=CONCAT(logTxt, '//[SESSION] RULE_', ruleId);
    IF dateCheck=0 THEN
    ...
    ELSE
      ...
      ELSE
        ...
        ELSE 
          ...
          -- creates a new istance only if the option "Enable One Award Achievement per Interval" is satisfied
            ...
            -- total number awarded to the player till now is less than the max_occurrences available for each player
      ...
      -- creates a new istance only if the option "Enable One Award Achievement per Interval" is satisfied
        ...
        IF (ruleInstanceEndDate IS NULL)
          OR (achievementIntervalTypeId IS NULL)
          OR (achievementIntervalTypeId=3 AND timestampdiff(day,ruleInstanceEndDate,transactionDate) > 0)
          OR (achievementIntervalTypeId=4 AND timestampdiff(week,ruleInstanceEndDate,transactionDate) > 0)
          OR (achievementIntervalTypeId=5 AND timestampdiff(month,ruleInstanceEndDate,transactionDate) > 0)
          OR (achievementIntervalTypeId=6 AND timestampdiff(quarter,ruleInstanceEndDate,transactionDate) > 0)
          ...
          THEN
            ...
            IF (IFNULL(amountAchieved,0) < IFNULL(maxoccurrences,9999999)) AND (IFNULL(amountAchievedPlayer,0) < IFNULL(maxoccurrencesPlayer,99999999)) THEN
              INSERT INTO gaming_rules_instances (client_stat_id, rule_id, is_current, is_achieved, date_created)
                SELECT clientId, ruleId, 1, 0, NOW();
              SET ruleInstanceId=LAST_INSERT_ID();  
            END IF;
      ...
      IF ruleInstanceId IS NOT NULL THEN
        ...
        INNER JOIN (
         SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                @logTxt2:=CONCAT(@logTxt2,' EV_', dvtbl2.event_id, '_', criteria_name,'+',case when criteria_name='LoginDuration' THEN varAmount ELSE 1 END) FROM (
         ...
         SELECT event_id, @logTxt:=CONCAT(@logTxt, GROUP_CONCAT(CASE WHEN res=1 THEN '' ELSE CONCAT(' EV_', event_id, '_', criteria_name,'=FAILED') END SEPARATOR '')),
          ...
          case when 
            (criteria_name='LoginDate' and 
              (
                (operator='on' and datediff(sessions_main.date_open, filter_date_from)=0)
                OR (operator='between' and sessions_main.date_open between filter_date_from and filter_date_to)
                OR (operator='until' and datediff(sessions_main.date_open, filter_date_from)<=0)
              )
            )
          then 1 else 
          ...
          from      
          sessions_main 
          join gaming_rules_instances ON sessions_main.session_id=transactionId  AND rule_instance_id=ruleInstanceId
          ...
          join gaming_re_event_criteria on gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
            AND gaming_re_event_criteria.event_type_id=4
          ...
          JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
           AND dvtbl2.res=1 AND gaming_re_event_criteria.criteria_name in ('LoginCount','LoginDuration')
          ...
          SET incr_result=IFNULL(incr_result,0)+case when criteria_name='LoginDuration' THEN varAmount ELSE 1 END;
  ...
  IF (isLogActive=1) AND (clientId IS NOT NULL) THEN 
    UPDATE gaming_event_rows SET client_id=clientId, `log`=CONCAT(`log`,' ',logTxt) WHERE event_table_id=4 AND elem_id=transactionId; 
  END IF;  
END root$$
