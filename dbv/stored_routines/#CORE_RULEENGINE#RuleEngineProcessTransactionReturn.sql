DROP procedure IF EXISTS `RuleEngineProcessTransactionReturn`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionReturn`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  DECLARE cur1 CURSOR FOR 
    SELECT gaming_rules.rule_id, gaming_game_plays.client_stat_id, gaming_game_plays.amount_total_base, 
            ...
            gaming_rules.player_max_occurrences, gaming_game_plays.timestamp,
      CASE WHEN (gaming_game_plays.timestamp between IFNULL(gaming_rules.start_date,'2001-01-01') and IFNULL(gaming_rules.end_date,'2399-01-01')) THEN 1 ELSE 0 END as date_check,
            CASE WHEN ((IFNULL(gaming_rules.player_selection_id,0)=0) OR EXISTS (SELECT player_selection_id FROM gaming_player_selections_player_cache 
                    ...
                    gaming_game_plays.license_type_id
        FROM 
          (SELECT gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, gaming_rules.max_occurrences, gaming_rules.player_max_occurrences, 
            ...
            WHERE gaming_events.event_type_id=3 AND is_active=1 AND is_hidden=0 GROUP BY gaming_rules.rule_id, gaming_rules.achievement_interval_type_id, gaming_rules.amount_achieved, 
          ...
          CROSS JOIN (SELECT * FROM gaming_game_plays WHERE game_play_id=transactionId and payment_transaction_type_id=13 and license_type_id<>3) as gaming_game_plays
  ...
  read_loop: LOOP
    SET done=FALSE;
    FETCH cur1 INTO ruleId, clientId, varAmount, ruleInstanceId, achievementIntervalTypeId, amountAchieved, maxoccurrences, maxoccurrencesPlayer, transactionDate, dateCheck, playerSelectionCheck, licenseTypeId;
    IF done THEN 
      LEAVE read_loop;
    END IF;
    SET logTxt=CONCAT(logTxt, '//[RETURN] RULE_', ruleId);
    IF dateCheck=0 THEN
      ...
      ELSE
        ...
        INNER JOIN (
         SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                   @logTxt2:=CONCAT(@logTxt2,' EV_', dvtbl2.event_id, '_', criteria_name,'+',
                   case when criteria_name='ReturnSum' THEN varAmount 
                        ELSE CASE WHEN criteria_name='ReturnNumRounds' AND licenseTypeId=6 THEN 1
                        ELSE 1 END END)
                   FROM (
         -- when the number of criteria matches with the number of verified criteria the ev
         SELECT event_id, CASE WHEN count(res) = sum(res) THEN 1 ELSE 0 END AS res FROM (
          ...
          case when 
            (criteria_name='ReturnCurrency' and 
              (
                (operator='in' AND CONCAT(',',filter_csv,',') LIKE CONCAT('%,',CAST(gaming_game_plays.currency_id AS CHAR),',%'))
                OR (operator='not_in' AND CONCAT(',',filter_csv,',') NOT LIKE CONCAT('%,',CAST(gaming_game_plays.currency_id AS CHAR),',%'))
              )
            )
          then 1 else 

          case when 
            (criteria_name='ReturnDate' and 
              (
                (operator='on' and datediff(gaming_game_plays.timestamp, filter_date_from)=0)
                OR (operator='between' and gaming_game_plays.timestamp between filter_date_from and filter_date_to)
                OR (operator='until' and datediff(gaming_game_plays.timestamp, filter_date_from)<=0)
              )
            )
          then 1 else 

          case when 
            (criteria_name='ReturnVertical' and 
              (
                (operator='in' AND CONCAT(',',filter_csv,',') LIKE CONCAT('%,',CAST(gaming_game_plays.license_type_id AS CHAR),',%'))
                OR (operator='not_in' AND CONCAT(',',filter_csv,',') NOT LIKE CONCAT('%,',CAST(gaming_game_plays.license_type_id AS CHAR),',%'))
              )
            )
          then 1 else     

          case when 
            (criteria_name='ReturnGame' and 
              (
                (operator='in' AND CONCAT(',',filter_csv,',') LIKE CONCAT('%,',CAST(gaming_game_plays.game_id AS CHAR),',%'))
                OR (operator='not_in' AND CONCAT(',',filter_csv,',') NOT LIKE CONCAT('%,',CAST(gaming_game_plays.game_id AS CHAR),',%'))
              )
            )
          then 1 else     

          case when 
            (criteria_name='ReturnGameCategory' and 
              (
                (operator='in' AND RuleEngineGameCategoriesCount(gaming_game_plays.game_id, filter_csv)>0)
                OR (operator='not_in' AND RuleEngineGameCategoriesCount(gaming_game_plays.game_id, filter_csv)=0)
              )
            )
          then 1 else   

          0 END END END END END END END AS res
          FROM      
          gaming_game_plays 
          JOIN gaming_rules_instances ON gaming_game_plays.game_play_id=transactionId  AND rule_instance_id=ruleInstanceId
          ...
          JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
            AND gaming_re_event_criteria.event_type_id=3
          LEFT JOIN gaming_game_categories_games ON gaming_game_categories_games.game_id=gaming_game_plays.game_id
          LEFT JOIN gaming_game_categories ON gaming_game_categories.game_category_id=gaming_game_categories_games.game_category_id
            ) dvtbl
          ...
          JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
           AND dvtbl2.res=1 AND gaming_re_event_criteria.criteria_name in ('ReturnSum','ReturnNumPlays','ReturnNumRounds')
          ...
          SET incr_result=IFNULL(incr_result,0) + 
            case when criteria_name='ReturnSum' THEN varAmount 
            ELSE CASE WHEN varAmount>0 THEN 1 ELSE 0 END
            END ;
