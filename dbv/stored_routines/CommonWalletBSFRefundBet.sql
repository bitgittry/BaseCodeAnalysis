DROP procedure IF EXISTS `CommonWalletBSFRefundBet`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletBSFRefundBet`(
  clientStatID BIGINT, cancelTransactionRef VARCHAR(80), transactionRef VARCHAR(80), minimalData TINYINT(1), canCommit TINYINT(1), OUT statusCode INT)
root: BEGIN
  ...
  CALL CommonWalletGeneralCheckTransactionProcessed(transactionRef, gameManufacturerName, 
	@transactionType, cwTransactionID, isAlreadyProcessed, statusCode);
  ...
  STRAIGHT_JOIN gaming_game_plays ON gaming_game_plays.game_play_id=gaming_cw_transactions.game_play_id
  STRAIGHT_JOIN gaming_payment_transaction_type AS transaction_type ON 
	transaction_type.name='Bet' AND 
	gaming_cw_transactions.payment_transaction_type_id=transaction_type.payment_transaction_type_id 
  WHERE gaming_cw_transactions.transaction_ref=cancelTransactionRef 
	AND gaming_cw_transactions.client_stat_id=clientStatID AND gaming_cw_transactions.game_manufacturer_id=gameManufacturerID 
  ...
  INSERT INTO gaming_cw_transactions (game_manufacturer_id, payment_transaction_type_id, amount, transaction_ref, round_ref, 
	game_ref, client_stat_id, game_play_id, timestamp, other_data, is_success, status_code, manual_update)
  SELECT gameManufacturerID, 20, refundAmount, transactionRef, NULL, 
	NULL, clientStatID, gamePlayIDReturned, NOW(), NULL, IF(statusCode=0,1,0), statusCode, 0;
