-- -------------------------------------
-- BonusGetFreeRoundRules.sql
-- -------------------------------------
DROP procedure IF EXISTS `BonusGetFreeRoundRules`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `BonusGetFreeRoundRules`(bonusRuleIDArray TEXT)
BEGIN
	...
	SELECT gaming_bonus_rules.bonus_rule_id, gaming_bonus_rules.name, gaming_bonus_rules.description, gaming_bonus_types.name AS bonus_type_name, priority, activation_start_date, activation_end_date, program_cost_threshold,
		...
		sb_bet_type_code, cash_transaction_multiplier, single_bet_allowed, accumulators_allowed, system_bets_allowed, accumulator_min_odd_per_selection, system_min_odd_per_selection,		gbrfr.num_rounds AS freeround_num_rounds, gbrfr.max_rounds_threshold AS freeround_max_rounds_threshold, gbrfr.num_rounds_given AS freeround_num_rounds_given,
		freeround_awarding_type.name AS freeround_awarding_type, gbrfr.remaining_funds_create_bonus,gaming_bonus_rules.is_free_bonus, no_loyalty_points, pre_expiry_days, redeem_threshold_enabled, voucher_code, restrict_by_voucher_code, 
		gaming_bonus_rules.comments,gaming_bonus_rules.linking_type, gaming_bonus_rules.redeem_threshold_on_deposit, gaming_bonus_rules.terms_and_conditions,award_bonus_max,
		date_eligable_check, currency_profile_id, game_weight_profile_id, sb_weight_profile_id, lotto_weight_profile_id, sportspool_weight_profile_id, bonus_custom_type_id, max_count_per_interval, is_generic, gaming_bonus_rules.awarded_times, gaming_bonus_rules.awarded_times_threshold,
	...
	JOIN gaming_bonus_rules_free_rounds AS gbrfr ON gaming_bonus_rules.bonus_rule_id=gbrfr.bonus_rule_id
	JOIN gaming_bonus_types_freeround_awarding AS freeround_awarding_type ON gbrfr.bonus_type_freeround_awarding_id=freeround_awarding_type.bonus_type_freeround_awarding_id;
	...
	JOIN gaming_bonus_rules ON bonus_rule_get_counter_id=bonusRuleGetCounterID AND gaming_bonus_rule_get_counter_rules.bonus_rule_id=gaming_bonus_rules.bonus_rule_id 
	...
	SELECT gbrfra.bonus_rule_id, gbrfra.min_bet AS freeround_min_bet, gbrfra.max_bet AS freeround_max_bet, gbrfra.max_win_total AS freeround_max_win_total, gaming_currency.currency_id, currency_code ,gbrfra.max_win AS freeround_max_win
	FROM gaming_bonus_rules_free_rounds_amounts AS gbrfra
	JOIN gaming_bonus_rule_get_counter_rules ON bonus_rule_get_counter_id=bonusRuleGetCounterID AND gbrfra.bonus_rule_id=gaming_bonus_rule_get_counter_rules.bonus_rule_id 
	JOIN gaming_currency ON  gbrfra.currency_id=gaming_currency.currency_id;
