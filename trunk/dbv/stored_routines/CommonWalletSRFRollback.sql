DROP procedure IF EXISTS `CommonWalletSRFRollback`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletSRFRollback`(gameSessionKey VARCHAR(40), clientStatID BIGINT, gameManufacturerName VARCHAR(80), cancelTransactionRef VARCHAR(80), transactionRef VARCHAR(80), roundRef BIGINT, gameRef VARCHAR(80), refundAmount DECIMAL(18, 5), canCommit TINYINT(1), OUT statusCode INT)
root: BEGIN
  
  DECLARE gameSessionID, sessionID, gameManufacturerID, gameRoundID, gamePlayID, operatorGameID, betCwTransactionID, clientStatIDCheck, gameID BIGINT DEFAULT -1;
  ...
  SELECT cw_transaction_id, transaction_type.name AS transaction_type 
  INTO cwTransactionID, transactionType
  FROM gaming_cw_transactions
  JOIN gaming_payment_transaction_type AS transaction_type ON gaming_cw_transactions.payment_transaction_type_id=transaction_type.payment_transaction_type_id 
  ...
  IF (cancelTransactionRef IS NULL OR cancelTransactionRef='') THEN
    ...
    JOIN gaming_game_plays ON gaming_game_rounds.round_ref=roundRef AND gaming_game_rounds.game_id=gameID AND 
