-- -------------------------------------
-- BonusGetManualRules.sql
-- -------------------------------------
DROP procedure IF EXISTS `BonusGetManualRules`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `BonusGetManualRules`(bonusRuleIDArray TEXT)
BEGIN
	...
	SELECT gaming_bonus_rules.bonus_rule_id, gaming_bonus_rules.name, gaming_bonus_rules.description, gaming_bonus_types.name AS bonus_type_name, priority, activation_start_date, activation_end_date, program_cost_threshold,
		...
		max_free_rounds_expiry_days_from_awarding AS manual_max_free_rounds_expiry_days_from_awarding, num_prerequisites_or
