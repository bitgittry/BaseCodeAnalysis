DROP procedure IF EXISTS `RuleEngineProcessTransactionStake`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `RuleEngineProcessTransactionStake`(transactionId BIGINT, isLogActive TINYINT(1))
root: BEGIN
  ...
  DECLARE cur1 CURSOR FOR 
    SELECT gaming_rules.rule_id, gaming_game_plays.client_stat_id, gaming_game_plays.amount_total_base, 
        ...
        FROM 
          ...
          CROSS JOIN (SELECT * FROM gaming_game_plays WHERE game_play_id=transactionId and payment_transaction_type_id=12 and license_type_id<>3) as gaming_game_plays
  ...
  read_loop: LOOP
    ...
    IF done THEN 
      LEAVE read_loop;
    END IF;
    SET logTxt=CONCAT(logTxt, '//[STAKE] RULE_', ruleId);
    IF dateCheck=0 THEN
        ...
        IF ruleInstanceId IS NOT NULL THEN
          ...
          INNER JOIN (
           SELECT gaming_re_event_criteria_instances.re_event_criteria_instance_id, gaming_re_event_criteria.criteria_name,
                  ...
                  ELSE CASE WHEN criteria_name='StakeNumRounds' AND licenseTypeId=6 THEN (select count(*) from gaming_game_plays INNER JOIN gaming_lottery_dbg_tickets ON gaming_lottery_dbg_tickets.lottery_coupon_id = gaming_game_plays.sb_bet_id AND gaming_game_plays.game_play_id=transactionId INNER JOIN gaming_lottery_participations ON gaming_lottery_participations.lottery_dbg_ticket_id=gaming_lottery_dbg_tickets.lottery_dbg_ticket_id)
                  ELSE 1 END END)
           ...
           SELECT event_id,  @logTxt:=CONCAT(@logTxt, GROUP_CONCAT(CASE WHEN res=1 THEN '' ELSE CONCAT(' EV_', event_id, '_', criteria_name,'=FAILED') END SEPARATOR '')),
            ...
            case when 
              (criteria_name='StakeVertical' and 
                (
            ...
            case when 
              (criteria_name='StakeGameCategory' and 
                (
            ...
            JOIN gaming_re_event_criteria ON gaming_re_event_criteria.re_event_criteria_id=gaming_re_event_criteria_config.re_event_criteria_id
              AND gaming_re_event_criteria.event_type_id=2
