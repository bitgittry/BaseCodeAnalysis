-- -------------------------------------
-- BonusGetDirectGWRuleByID.sql
-- -------------------------------------
DROP procedure IF EXISTS `BonusGetDirectGWRuleByID`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `BonusGetDirectGWRuleByID`(bonusRuleID BIGINT)
BEGIN
	...
	SELECT gaming_bonus_rules_direct_gvs_amounts.bonus_rule_id, amount AS directgw_amount, gaming_currency.currency_id, currency_code 
	FROM gaming_bonus_rules_direct_gvs_amounts
	JOIN gaming_currency ON gaming_bonus_rules_direct_gvs_amounts.currency_id=gaming_currency.currency_id
	WHERE bonus_rule_id = bonusRuleID;
END$$
