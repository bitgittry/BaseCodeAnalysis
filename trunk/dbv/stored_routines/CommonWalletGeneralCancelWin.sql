DROP procedure IF EXISTS `CommonWalletGeneralCancelWin`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletGeneralCancelWin`(
  gameSessionID BIGINT, clientStatID BIGINT, gameManufacturerName VARCHAR(80), cancelTransactionRef VARCHAR(80), transactionRef VARCHAR(80), 
  roundRef BIGINT, gameRef VARCHAR(80), winToCancelAmount DECIMAL(18, 5), canCommit TINYINT(1), manualUpdate TINYINT(1), OUT statusCode INT)
root: BEGIN

  DECLARE sessionID, gameRoundID, gamePlayID, operatorGameID, winCwTransactionID, clientStatIDCheck, gameID BIGINT DEFAULT -1;
  DECLARE winTotal DECIMAL(18, 5) DEFAULT 0;
  DECLARE gamePlayIDReturned, cwTransactionID, gameManufacturerID BIGINT DEFAULT NULL;
  DECLARE isAlreadyProcessed, winIsSuccess TINYINT(1) DEFAULT 0;
  ...
  IF (clientStatID IS NULL) THEN
    SELECT gaming_game_manufacturers.game_manufacturer_id, gaming_cw_transactions.client_stat_id, 
		IFNULL(gameRef, gaming_cw_transactions.game_ref), IFNULL(winToCancelAmount, gaming_cw_transactions.amount)
    INTO gameManufacturerID, clientStatID, gameRef, winToCancelAmount
    ...
    JOIN gaming_payment_transaction_type AS transaction_type ON transaction_type.name='Win' AND 
  ...
  SET @transactionType='WinCancelled'; 
  ...
  IF (cancelTransactionRef IS NULL OR cancelTransactionRef='') THEN
		...
		SELECT gaming_game_plays.game_round_id, gaming_game_plays.game_play_id, gaming_game_plays.amount_total, 
			IFNULL(winToCancelAmount, gaming_cw_transactions.amount), gaming_cw_transactions.exchange_rate 
		INTO gameRoundID, gamePlayID, winTotal, winToCancelAmount, exchangeRate
  ...
  IF (gamePlayID=-1) THEN
    ...
    FROM gaming_game_manufacturers
    JOIN gaming_payment_transaction_type AS transaction_type ON gaming_game_manufacturers.name=gameManufacturerName AND transaction_type.name='WinCancelled';
  ...
  CALL PlaceWinCancel(gamePlayID, gameSessionID, winToCancelAmount, transactionRef, gamePlayIDReturned, statusCode);
