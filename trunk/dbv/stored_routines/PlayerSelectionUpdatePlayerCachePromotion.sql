DROP procedure IF EXISTS `PlayerSelectionUpdatePlayerCachePromotion`;

DELIMITER $$

CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlayerSelectionUpdatePlayerCachePromotion`(clientStatID BIGINT)
BEGIN
  -- Optimized
  ...
  SELECT gaming_promotions.player_selection_id, clientStatID, 
		 (SELECT @a:= IFNULL(cache.player_in_selection, PlayerSelectionIsPlayerInSelection(gaming_promotions.player_selection_id,clientStatID))) AS cache_new_value, 
		 IF(@a=1, DATE_ADD(NOW(), INTERVAL (SELECT player_minutes_to_expire FROM gaming_player_selections WHERE player_selection_id = gaming_promotions.player_selection_id)  MINUTE), cache.expiry_date),		  
		 curDateTemp
  ...
  WHERE achievement_end_date >= curDateTemp AND gaming_promotions.is_active=1 AND gaming_promotions.is_child=0 AND cache.player_in_selection IS NULL 
