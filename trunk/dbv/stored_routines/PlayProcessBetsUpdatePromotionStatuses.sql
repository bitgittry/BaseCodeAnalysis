DROP procedure IF EXISTS `PlayProcessBetsUpdatePromotionStatuses`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PlayProcessBetsUpdatePromotionStatuses`(gamePlayProcessCounterID BIGINT)
root: BEGIN

  -- Forced a straight join but still to optimize all joins on trunk 
    
  DECLARE promotionAwardPrizeOnAchievementEnabled, singlePromotionsEnabled, noMoreRecords TINYINT(1) DEFAULT 0;
  DECLARE contributeAllowMultiple, contributeRealMoneyOnly, sportsBookActive TINYINT(1) DEFAULT 1;
  ...
  DECLARE promotionAwardOnAchievementCursor CURSOR FOR 
    ...
    JOIN gaming_promotions ON pps.promotion_id=gaming_promotions.promotion_id AND 
	...
	WHERE (IFNULL(gaming_promotions.award_num_players_per_occurence, 999999999999) > IFNULL(dates.awarded_prize_count, 0))
  ...
  UPDATE gaming_game_plays_process_counter_rounds 
  JOIN gaming_game_rounds ON
	   gaming_game_plays_process_counter_rounds.game_play_process_counter_id=@gamePlayProcessCounterID AND
	   gaming_game_plays_process_counter_rounds.game_round_id=gaming_game_rounds.game_round_id AND gaming_game_rounds.game_round_type_id=1 
  JOIN gaming_promotions_player_statuses AS gpps FORCE INDEX (client_open_promos) ON
    gpps.client_stat_id=gaming_game_rounds.client_stat_id AND gpps.is_active=1 AND gpps.is_current=1 AND gpps.end_date<dateTimeNow
  ...
  SELECT gaming_promotions.promotion_id, child_promotion.promotion_id, gaming_game_rounds.client_stat_id, gaming_promotions.priority, NOW(), gcs.currency_id, promotionGetCounterID,
  ...
  AND gppo.client_stat_id = gaming_game_rounds.client_stat_id AND gppo.opted_in = 1 AND gppo.awarded_prize_count < IFNULL(gaming_promotions.award_num_times_per_player, 99999999999)
  JOIN gaming_promotions_recurrence_dates AS recurrence_dates FORCE INDEX (promotion_active_current) ON 
    recurrence_dates.promotion_id=gaming_promotions.promotion_id AND recurrence_dates.is_active=1 AND recurrence_dates.is_current=1 AND 
    (gaming_game_rounds.date_time_start BETWEEN recurrence_dates.start_date AND recurrence_dates.end_date) 
    AND recurrence_dates.awarded_prize_count < IFNULL(gaming_promotions.award_num_players_per_occurence, 999999999999)
  JOIN gaming_client_stats AS gcs ON gcs.client_stat_id = gaming_game_rounds.client_stat_id AND gcs.is_active=1
  LEFT JOIN gaming_player_selections_player_cache AS CS ON CS.client_stat_id=gaming_game_rounds.client_stat_id AND gaming_promotions.player_selection_id = CS.player_selection_id
  ...
  LEFT JOIN gaming_promotions_player_statuses AS gpps ON gpps.promotion_id=gaming_promotions.promotion_id AND gpps.client_stat_id=gaming_game_rounds.client_stat_id AND gpps.is_current=1
  WHERE (gaming_promotions.achievement_end_date>=NOW() AND ((gaming_promotions.need_to_opt_in_flag=0 AND IFNULL(gppo.creation_counter_id,0)=promotionGetCounterID) 
	...
	AND (IFNULL(CS.player_in_selection, PlayerSelectionIsPlayerInSelectionCached(gaming_promotions.player_selection_id, gaming_game_rounds.client_stat_id))=1 OR gaming_promotions.auto_opt_in_next = 1)
  ...
  (
    ...
    FROM
    (
      SELECT gaming_game_rounds.game_round_id, pps.promotion_id, pps.promotion_player_status_id, ppsd.promotion_player_status_day_id, gaming_promotions_games.promotion_wgr_req_weight, 
        ...
        LEAST(IFNULL(wager_restrictions.max_wager_contibution, 1000000000000), LEAST(IFNULL(wager_restrictions.max_wager_contibution_before_weight, 1000000000000), IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, win_real-bet_real, win_total-bet_total))*gaming_promotions_games.promotion_wgr_req_weight, IF(gaming_promotions.is_single AND gppa.max_cap IS NOT NULL, gppa.max_cap, 1000000000000)) AS win_total, 
        LEAST(IFNULL(wager_restrictions.max_wager_contibution, 1000000000000), LEAST(IFNULL(wager_restrictions.max_wager_contibution_before_weight, 1000000000000), IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, bet_real-win_real, bet_total-win_total))*gaming_promotions_games.promotion_wgr_req_weight, IF(gaming_promotions.is_single AND gppa.max_cap IS NOT NULL, gppa.max_cap, 1000000000000)) AS loss_total
      FROM gaming_game_plays_process_counter_rounds 
      JOIN gaming_game_rounds ON
        gaming_game_plays_process_counter_rounds.game_play_process_counter_id=@gamePlayProcessCounterID AND
        gaming_game_plays_process_counter_rounds.game_round_id=gaming_game_rounds.game_round_id AND gaming_game_rounds.game_round_type_id=1 
      ...
      JOIN gaming_promotions_player_statuses AS pps FORCE INDEX (promotion_client_active_current) ON
        (pps.promotion_id=gaming_promotions.promotion_id AND pps.client_stat_id=gaming_game_rounds.client_stat_id AND pps.is_active=1 AND pps.is_current=1 AND (IF(gaming_promotions.recurrence_enabled = 1, (gaming_game_rounds.date_time_start BETWEEN pps.start_date AND pps.end_date), 1=1))) AND pps.requirement_achieved=0 
      ...
      LEFT JOIN gaming_promotions_achievement_amounts ON
		...
		(gaming_promotions.is_single=0 OR ( 
			(gaming_promotions.promotion_achievement_type_id=1 AND (IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, bet_real, bet_total)*gaming_promotions_games.promotion_wgr_req_weight)>=gaming_promotions_achievement_amounts.amount) OR
			(gaming_promotions.promotion_achievement_type_id=2 AND (IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, win_real-bet_real, win_total-bet_total)*gaming_promotions_games.promotion_wgr_req_weight)>=gaming_promotions_achievement_amounts.amount)
		))
	  ...
	  LEFT JOIN gaming_game_rounds_promotion_contributions AS ggrpc ON ggrpc.game_round_id=gaming_game_rounds.game_round_id 
      ...
      WHERE
		...
		(contributeAllowMultiple OR ggrpc.game_round_id IS NULL) AND	
      ...
      ORDER BY gaming_game_rounds.game_round_id, pps.priority ASC, pps.opted_in_date DESC 
    ) AS PP
  ) AS PP
    ...
    (
      ...
      (
        SELECT gaming_game_rounds.game_round_id, pps.promotion_id, pps.promotion_player_status_id, ppsd.promotion_player_status_day_id, 
            ...
            LEAST( IFNULL(wager_restrictions.max_wager_contibution, 10000000000), 
                   IF(gaming_promotions.is_single AND gppa.max_cap IS NOT NULL, gppa.max_cap, 1000000000000),
                   ( LEAST(IFNULL(wager_restrictions.max_wager_contibution_before_weight, 10000000000),
                           IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, win_real - bet_real, win_total - bet_total)) * 
                     IF(sb_weights.weight IS NULL OR sb_weights_multiple.weight IS NULL, 
                        COALESCE(sb_weights.weight, sb_weights_multiple.weight), 
                        LEAST(sb_weights.weight, sb_weights_multiple.weight)
                       )
            ...
            LEAST( IFNULL(wager_restrictions.max_wager_contibution, 10000000000), 
                   IF(gaming_promotions.is_single AND gppa.max_cap IS NOT NULL, gppa.max_cap, 1000000000000),
                   ( LEAST(IFNULL(wager_restrictions.max_wager_contibution_before_weight, 10000000000),
                           IF(contributeRealMoneyOnly OR gaming_promotions.wager_req_real_only, bet_real-win_real, bet_total-win_total)) * 
                     IF(sb_weights.weight IS NULL OR sb_weights_multiple.weight IS NULL, 
                        COALESCE(sb_weights.weight, sb_weights_multiple.weight), 
                        LEAST(sb_weights.weight, sb_weights_multiple.weight)
                       )
        ...
        JOIN gaming_game_rounds ON
            gaming_game_plays_process_counter_rounds.game_play_process_counter_id=@gamePlayProcessCounterID AND
            gaming_game_plays_process_counter_rounds.game_round_id=gaming_game_rounds.game_round_id AND
        ...
        (
            ...
            FROM gaming_game_plays_process_counter_rounds AS ggppcr
            JOIN gaming_game_rounds AS ggr ON
                ggppcr.game_play_process_counter_id=@gamePlayProcessCounterID AND
                ggppcr.game_round_id=ggr.game_round_id AND
        ...
        WHERE
            IFNULL(ppsd.daily_requirement_achieved, 0) = 0 AND
            (   singlePromotionsEnabled OR gaming_promotions.is_single = 0 ) AND
            (   contributeAllowMultiple OR ggrpc.game_round_id IS NULL ) AND	
            (   pps.promotion_recurrence_date_id IS NULL OR
                gaming_promotions_status_days.promotion_recurrence_date_id IS NULL OR 
                pps.promotion_recurrence_date_id = gaming_promotions_status_days.promotion_recurrence_date_id
            ) AND        
  ...
  (
    ...
    STRAIGHT_JOIN gaming_promotions_player_statuses AS pps ON 
      promotion_contributions.game_play_process_counter_id=@gamePlayProcessCounterID AND
      promotion_contributions.promotion_player_status_id=pps.promotion_player_status_id
    STRAIGHT_JOIN gaming_promotions_player_statuses_daily AS ppsd ON promotion_contributions.promotion_player_status_day_id=ppsd.promotion_player_status_day_id
