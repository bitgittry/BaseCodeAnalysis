...
DROP function IF EXISTS `UpdatePromotionOccurrences`;

DELIMITER $$

CREATE DEFINER=`bit8_admin`@`127.0.0.1` FUNCTION `UpdatePromotionOccurrences`(recurrenceType VARCHAR(10), promotionID BIGINT(20), weekInterval int(11), durationMinutes BIGINT(10), startDate DATETIME, endDate DATETIME, startTime VARCHAR(10)) RETURNS int(11)
BEGIN
...
SET @latestDate = concat(date(@latestDate), ' 00:00:00');  

CASE recurrenceType  
	WHEN 1 THEN  
		SET @latestDate = IF(@recurrenceCount = 0 ,@latestDate,DATE_ADD(@latestDate, INTERVAL weekInterval DAY)); 
