-- -------------------------------------
-- PromotionGetSportBookWeights.sql
-- -------------------------------------

DROP procedure IF EXISTS `PromotionGetSportBookWeights`;

DELIMITER $$

CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `PromotionGetSportBookWeights`(promotionID BIGINT)
BEGIN
	
  SELECT sp.sb_sport_id, sp.name, sp.ext_sport_id, sp.game_manufacturer_id, crit.eligibility_criterias_id 
  FROM gaming_promotions_wgr_sb_eligibility_criterias AS crit
  JOIN gaming_promotions_wgr_sb_profile_selections AS sel ON crit.eligibility_criterias_id = sel.eligibility_criterias_id
  LEFT JOIN gaming_sb_sports AS sp ON sel.sb_entity_id = sp.sb_sport_id
  WHERE crit.promotion_id = promotionID AND sel.sb_entity_type_id = 1
  ...
  WHERE crit.promotion_id = promotionID AND sel.sb_entity_type_id = 2
  ...
  WHERE crit.promotion_id = promotionID AND sel.sb_entity_type_id = 3
  ...
  WHERE crit.promotion_id = promotionID AND sel.sb_entity_type_id = 4
  ...
  WHERE crit.promotion_id = promotionID AND sel.sb_entity_type_id = 5
  ...
  FROM gaming_promotions_wgr_sb_eligibility_criterias AS crit
  JOIN gaming_promotions_wgr_sb_weights AS weight ON crit.eligibility_criterias_id = weight.eligibility_criterias_id 
  WHERE crit.promotion_id = promotionID AND weight.sb_weight_range_id IS NULL AND weight.min_odd > 0
  ...
  FROM gaming_promotions_wgr_sb_weights AS weight
  ...
  WHERE weight.promotion_id = promotionID AND weight.sb_weight_range_id IS NOT NULL AND weight.min_odd > 0
  ORDER BY weight.sb_entity_type_id, weight.sb_entity_id, weight.odd_order;
  ...
  WHERE weight.promotion_id = promotionID AND weight.sb_multiple_type_id IS NOT NULL
  ORDER BY weight.sb_entity_type_id, weight.sb_entity_id, weight.odd_order;
