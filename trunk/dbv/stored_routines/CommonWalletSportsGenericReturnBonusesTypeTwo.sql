DROP procedure IF EXISTS `CommonWalletSportsGenericReturnBonusesTypeTwo`;

DELIMITER $$
CREATE DEFINER=`bit8_admin`@`127.0.0.1` PROCEDURE `CommonWalletSportsGenericReturnBonusesTypeTwo`(
...
BEGIN
	...
	STRAIGHT_JOIN gaming_game_plays_sb AS bet_play_sb FORCE INDEX (game_round_id) ON 
		bet_play_sb.game_round_id=gaming_game_plays_sb.game_round_id AND 
		bet_play_sb.payment_transaction_type_id=12 AND
        -- parition filtering
	...
	STRAIGHT_JOIN gaming_game_plays_sb_bonuses AS bet_sb_bonuses FORCE INDEX (PRIMARY) ON 
		bet_play_sb.game_play_sb_id=bet_sb_bonuses.game_play_sb_id
	...
	IF (cancelledBonusNow + cancelledBonusWinLockedNow > 0 OR @numBonuses > 0) THEN
		...
		SELECT 
			GREATEST(gaming_game_plays_sb.game_play_sb_id, (@adjustmentRatio:=(gaming_game_plays_sb.amount_total/bet_play_sb.amount_total*-1))), 
			bet_sb_bonuses.bonus_instance_id, 
			bet_sb_bonuses.bet_bonus_win_locked * @adjustmentRatio, 
			bet_sb_bonuses.bet_real * @adjustmentRatio, 
			bet_sb_bonuses.bet_bonus * @adjustmentRatio, 
			bet_sb_bonuses.wager_requirement_non_weighted * @adjustmentRatio, 
			bet_sb_bonuses.wager_requirement_contribution_before_real_only * @adjustmentRatio, 
			bet_sb_bonuses.wager_requirement_contribution * @adjustmentRatio, 
			bet_sb_bonuses.wager_requirement_contribution_cancelled * @adjustmentRatio
		FROM gaming_game_plays_sb FORCE INDEX (game_play_id)
		STRAIGHT_JOIN gaming_game_plays_sb AS bet_play_sb FORCE INDEX (game_round_id) ON 
			bet_play_sb.game_round_id=gaming_game_plays_sb.game_round_id AND 
			bet_play_sb.payment_transaction_type_id=12 AND
		...
		UPDATE 
		(
			SELECT cancel_sb_bonuses.bonus_instance_id, 
				COUNT(bet_play_sb.amount_total=gaming_game_plays_sb.amount_total) AS num_rounds, 
		...
		SET 
			/**
			* Commented because bonus_amount_remaining was updated in SP: CommonWalletSportsGenericReturnFundsTypeTwo
			* -- gbi.bonus_amount_remaining = bonus_amount_remaining + IF(gbi.is_lost=1 OR gbi.is_secured=1, 0, CancelBonus.amount_bonus), 
			*
			* Commented because current_win_locked_amount was updated in SP: CommonWalletSportsGenericReturnFundsTypeTwo
			...
			*/
			...
			gbi.open_rounds  = ABS(gbi.open_rounds - CancelBonus.num_rounds),
			gbi.is_active = IF(gbi.is_secured OR gbi.is_lost=1, gbi.is_active, 1),
			gbi.is_used_all = IF(
			(bonus_amount_remaining + CancelBonus.amount_bonus + gbi.current_win_locked_amount + CancelBonus.amount_bonus_win_locked) = 0
				AND (gbi.open_rounds - CancelBonus.num_rounds) = 0, 
			1, gbi.is_used_all);
		...
		STRAIGHT_JOIN gaming_game_plays_sb_bonuses AS cancel_sb_bonuses ON 
			gaming_game_plays_sb.game_play_sb_id=cancel_sb_bonuses.game_play_sb_id
		STRAIGHT_JOIN gaming_bonus_instances ON 
			cancel_sb_bonuses.bonus_instance_id=gaming_bonus_instances.bonus_instance_id AND 
            gaming_bonus_instances.is_secured=1
		WHERE gaming_game_plays_sb.game_play_id=cancelGamePlayID AND
			-- parition filtering
			(gaming_game_plays_sb.game_play_sb_id BETWEEN minGamePlaySBID AND maxGamePlaySBID);
